...
def action_after_open(x_coordinate, y_coordinate, width, height, image_path, screen_area, action, image_name,
                      folder_name, flop_deck, stack_collection):
    if check_is_flop(screen_area, image_name, folder_name, flop_deck, stack_collection): return
    if action == 'open':
        if check_is_fold(screen_area, x_coordinate, y_coordinate, width, height, image_path): return
    if check_is_action_buttons(screen_area): return


def save_element(screen_area, element_name):
    element_area = get_element_area(screen_area, element_name)
    for item in get_element_data(element_area):
...
def check_is_limp_available(screen_area, element):
    element_area = save_element(screen_area, 'limp_area')
    if image_processing.search_element(element_area, element, 'limp/'):
        return True
    return False


def check_is_flop(screen_area, image_name, folder_name, flop_deck, stack_collection):
    element_area = save_element(screen_area, 'green_board_area')
    if image_processing.search_element(element_area, ['green_board'], 'green_board/') is False:
        last_row = session_log.get_last_row_from_log_session(str(screen_area))
        hand = last_row[0][0]
        stack = last_row[0][1]
        action = last_row[0][3]
        is_headsup = last_row[0][4]
        if is_headsup == 0 and headsup.search_opponent_card(screen_area, is_postflop=True)[0] == 1:
            is_headsup = 1
            session_log.update_is_headsup_postflop(str(screen_area), is_headsup)
        if len(hand) == 4:
            if action == 'open':
                stack = current_stack.get_actual_stack(screen_area, stack_collection, folder_name)
            flop.make_flop_decision(str(screen_area), hand, image_name, folder_name, stack, action, is_headsup,
                                    flop_deck)
        else:
...
def check_is_action_buttons(screen_area):
    row = session_log.get_last_row_from_log_session(screen_area)
    try:
        reaction_to_opponent = get_reaction_to_opponent(row)[0]['reaction_to_opponent']
        if not isinstance(reaction_to_opponent, str):
            reaction_to_opponent = 'fold'
    except:
        reaction_to_opponent = 'fold'
    last_opponnet_action = image_processing.search_last_opponent_action(screen_area)
    ...
    if reaction_to_opponent == 'push':
        ...
        keyboard.press('f')
    session_log.update_action_log_session(reaction_to_opponent, str(screen_area))


def check_is_fold(screen_area, x_coordinate, y_coordinate, width, height, image_path):
    last_hand = session_log.get_last_row_from_log_session(str(screen_area))[0]['hand']
    last_hand = last_hand[:4]
    image_processing.imaging(x_coordinate, y_coordinate, width, height, image_path, str(screen_area))
    cur_hand = image_processing.search_cards(str(screen_area), image_processing.get_cards(), 4)
    if last_hand != cur_hand:
        folder_name = IMAGES_FOLDER + '/' + str(datetime.datetime.now().date())
        image_name = str(math.floor(time.time())) + ".png"
        session_log.update_action_log_session('end', str(screen_area))
        determine_position.save_blind_image(screen_area, image_name, folder_name)
        current_stack.save_stack_image(screen_area, image_name, folder_name)
        session_log.check_conditions_before_insert(cur_hand, int(screen_area), current_stack.get_stack_images())
        logic.get_decision(screen_area)
        return True


def get_element_area(screen_area, element):
...
def get_element_data(screen_area):
    db = postgresql.open(db_conf.connection_string())
...
def get_reaction_to_opponent(row):
...
def reaction_to_opponent_sitout(screen_area):
    keyboard.press('q')
